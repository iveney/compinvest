...
Modified to choose the portfolio with highest sharpe ratio, sort the symbols
@modified: Zigang Xiao
...
'''
Find the best portfolio and sort the symbols
'''
na_sharpes = np.array(lf_returns) / np.array(lf_std)
port_sharpe = zip(lna_portfolios, na_sharpes)
best_port = argmax(pairs)
indices = range(len(ls_symbols))
indices_weight = zip(indices, best_port)
indices_weight_sorted = sorted(indices_weight, key = operator.itemgetter(1))
# symbols_weight_sorted = [(ls_symbols[i]), w) for i, w in symbols_weight_sorted]

'''
Plot the stocks with weight > 0.001
'''
indices_to_plot = filter(lambda pair: pair[1] > 0.001, indices_weight_sorted)
indices_to_plot = [i for i, w in indices_to_plot]
symbols_name = [ls_symbols[i] for i in indices_to_plot]

ldt_timestamps_combined = ldt_timestamps + ldt_timestamps_test
na_price_combined = np.concatenate([df_close.values.copy(),
                                    df_close_test.values.copy()])
na_price_normalized = na_price_combined / na_price_combined[0, :]
plt.clf()
plt.plot(ldt_timestamps_combined, na_price_normalized[:, indices_to_plot])
plt.legend(symbols_name)
